...
- **Titles.** Boards, folders and scenes can be called anything - "MVP", "UI",
  "Checkout (v2) 🛒". Rename in the sidebar (scenes too, now) edits a `title`;
  the file name, folder key and scene directory stay the slug agents address.
  No title = the Title-Cased slug, as before. Stored on the board JSON, the
  `_folders.json` entry and the brief's YAML front matter; shown everywhere the
  object is named and carried by the manifest and `npx marver boards`.

### Fixed

- **Drops land where the seam shows.** A board grabbed by its icon and dragged
  between two folder boards landed after the folder; releases in blank space did
  nothing. The resolver now reads the pointer against the rendered rows, the
  release applies the seam on screen, and a release outside the panel cancels.
- A project reached through a symlink (macOS `/var`, a linked projects folder)
  lost its live updates; the CLI now resolves the root's real path.

### Changed

- `boards/rename` takes `{ from, to?, title?, baseHash? }` (title-only, CAS);
  new `scenes/rename { scene, title }`; `sh:scenes` relabels scenes without
  re-keying iframes.
