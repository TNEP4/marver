...
export function goalText(packet: JobPacket): string {
  return [
    ...
    '',
    'Your FINAL message is your reply to the comment thread (the system posts it for you).',
    'REPLY RULES (important - the thread shows RAW text, so markdown appears as literal characters):',
    '- PLAIN TEXT ONLY. No markdown whatsoever: no **bold**, no `backticks`, no #headings, no - bullet',
    '  lists, no tables. Line breaks (blank lines) are your ONLY formatting.',
    '- MATCH THE HUMAN. Mirror their energy, tone, and LENGTH. A short casual one-liner gets a short',
    '  casual one-liner back. Do not write an essay for a small ask. Usually 1-2 short lines is right.',
    '- Say what you did in plain words; mention a file path only if it genuinely helps. Do not resolve',
    '  the thread.',
    '',
    'JOB PACKET:',
