...
[${NAME}] design/ already exists in this repo and does not look like a ${NAME} workspace.
  ...
  // Root AGENTS.md: Codex and most non-Claude agents read AGENTS.md, and it has no
  // import syntax - a plain pointer sentence delivers the same contract. Same rules
  // as CLAUDE.md: create it, or append one line; never touch the human's own bytes.
  const rootAgentsMd = join(root, 'AGENTS.md')
  const pointerLine = `Before any design or canvas work, read design/AGENTS.md and follow it - it is binding.`
  if (!existsSync(rootAgentsMd)) {
    writeFileSync(rootAgentsMd, `# ${NAME}\n\nThis repo designs with ${NAME}. ${pointerLine}\n`)
    created.push('AGENTS.md')
  } else {
    const cur = readFileSync(rootAgentsMd, 'utf8')
    if (!cur.includes('design/AGENTS.md')) {
      writeFileSync(rootAgentsMd, cur.replace(/\n*$/, '') + `\n\n${pointerLine}\n`)
  ...
  // Same enumeration the dev-boot staleness probe reads, so the two can never drift.
  for (const { rel, body } of enumerateInstructionTemplates(templates)) writeManaged(rel, body)
  ...
  write('slides.md', [
    ...
    '  no stock; illustrations in the product\'s line style only.',
  ...
  const sceneDirs = existsSync(join(design, 'scenes'))
    ? readdirSync(join(design, 'scenes'), { withFileTypes: true }).filter((d) => d.isDirectory() && !d.name.startsWith('_')).length
    : 0
  if (opts.demo && sceneDirs === 0 && !existsSync(join(design, 'scenes', 'demo'))) {
