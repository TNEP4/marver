...
describe('activityJobs - who a fresh batch mails', () => {
  ...
  it('a fresh reply mails prior participants, never the author, throttled per window', () => {
    ...
    expect([...jobs.values()]).toEqual([{ template: 'reply', recipient: A, actor: undefined, snippet: 'x', link: '/#/b/main?c=t1' }])
  ...
  it('history imports never mail; roots mail only their mentions', () => {
    ...
    expect([...activityJobs('main', [root], [root], all, now).values()]).toEqual([{ template: 'mentioned', recipient: C, actor: undefined, snippet: 'x', link: `/#/b/main?c=${root.commentId}` }])
  })
