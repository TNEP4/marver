#!/usr/bin/env node
/**
 * Packed-package smoke: proves packaging completeness, optimizeDeps.exclude,
...
const reg = await get('/@fs/' + join(app, 'node_modules/@marver-design/marver/src/client/frame-host/registry.ts').replaceAll('\\', '/'))
...
for (const mod of shellRoots) {
  const r = await get('/@fs/' + join(app, `node_modules/@marver-design/marver/src/client/${mod}`).replaceAll('\\', '/'))
